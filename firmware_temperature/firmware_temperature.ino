...
void setup() {
  ...
  // 2. ENVOYER AU SERVEUR (HTTPS POST)
  ...
  Serial.print("Envoi Temperature: "); Serial.println(t);
  
  http.begin(client, urlData);
  http.addHeader("Content-Type", "application/json");
  // Construction manuelle du JSON pour éviter une bibliothèque en plus
  String json = "{\"temperature\": " + String(t) + "}"; 
  
  int httpResponseCode = http.POST(json);
  if (httpResponseCode > 0) Serial.println("Données envoyées !");
  else Serial.printf("Erreur envoi: %d\n", httpResponseCode);
  http.end();
